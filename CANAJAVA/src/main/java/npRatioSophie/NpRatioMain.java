...
public class NpRatioMain {
	...
	public static void main(String[] args) {
		...
		TCanvas canvas = new TCanvas("neutron proton ratio depending on time", 800, 800);
		// TCanvas canvas1 = new TCanvas("neutron proton ratio depending on
		// temperature", 800, 800);
		 ...
		 * 
		 * np = npRatio.equilibrium_npRatio(T); npRatio.setnewg(T); time =
		 * npRatio.time(T);
		 * 
		 * if (Math.log10(np2) >= -2) {
		 * 
		 * // System.out.println(Math.log10(np)); // grA.addPoint(Math.log10(time),
		 * Math.log10(np), 0, 0); grA1.addPoint(Math.log10(T), Math.log10(np), 0, 0);
		 * 
		 * if (T <= limit_equilibrium) {
		 * 
		 * np2 = npRatio.neutron_decay(time - time_nd);
		 * 
		...
		for (double t = start_time; t < limit_time; t += 10) {
			...
			if (Math.log10(np2) >= -2) {
				...
				grA.addPoint(Math.log10(t), Math.log10(np), 0, 0);

				// grA1.addPoint(Math.log10(T), Math.log10(np), 0, 0);

				if (T <= limit_equilibrium) {

					np2 = npRatio.neutron_decay(t - time_nd);
